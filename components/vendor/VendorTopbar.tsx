...
import {
  Menu, Bell, Search, ChevronDown, LogOut, User, Settings, Moon, Sun,
  Store, ExternalLink,
} from "lucide-react";
    ...
    <nav className="hidden sm:flex items-center gap-1 text-sm">
      {crumbs.map((crumb, i) => (
        <span key={crumb.href} className="flex items-center gap-1">
          {i < crumbs.length - 1 ? (
            <>
              <Link href={crumb.href} className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition-colors">
                {crumb.label}
              </Link>
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="relative p-2 rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 transition-colors"
    ...
    <div ref={ref} className="relative">
      ...
      >
        ...
        <div className="hidden md:block text-left">
          <p className="text-sm font-semibold text-gray-900 dark:text-white leading-none">{vendorName}</p>
          <p className="text-[10px] text-gray-400 leading-none mt-0.5">Vendor</p>
        </div>
      ...
      {open && (
        <div className="absolute right-0 top-12 z-50 w-52 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-2xl py-1">
          ...
          <Link
            ...
            className="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
          >
            <Store className="w-4 h-4 text-gray-400" /> KYC Status
          </Link>
  ...
  return (
    <header className="flex items-center gap-4 h-14 px-4 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex-shrink-0">
      ...
      <div className="flex items-center gap-1">
        ...
        <button
          onClick={toggleDark}
          className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 transition-colors"
