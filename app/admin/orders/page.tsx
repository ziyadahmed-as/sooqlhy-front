...
function OrdersContent() {
  ...
  return (
    <AdminPageWrapper title="Order Management" subtitle={`${count} orders`}
      ...
      <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search by order ID, customer, tracking..."
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {STATUS_OPTIONS.map((s) => (
            <button key={s} onClick={() => { setStatusFilter(s); setPage(1); }}
              className={cn("px-2.5 py-1.5 rounded-lg text-xs font-semibold border transition-colors", statusFilter === s ? "bg-slate-800 text-white border-slate-800" : "bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-slate-400")}>
              {s || "All"}
            </button>
          ))}
        </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
        {loading ? <div className="p-4 space-y-2">{[...Array(6)].map((_, i) => <div key={i} className="h-12 bg-gray-100 dark:bg-gray-800 rounded-lg animate-pulse" />)}</div>
          : orders.length === 0 ? <EmptyState title="No orders found" description="Try adjusting filters." icon={<ShoppingBag className="h-10 w-10" />} />
          : (
                  ...
                  {orders.map((o: any) => (
                    <tr key={o.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-500 dark:text-gray-400 max-w-[150px]">
                        <div className="flex items-center gap-1 truncate"><MapPin className="w-3 h-3 flex-shrink-0" /><span className="truncate">{o.shipping_address?.city ?? "—"}</span></div>
                      </td>
...
export default function AdminOrdersPage() {
  return <Suspense fallback={<div className="p-8 text-center text-sm text-gray-400">Loading...</div>}><OrdersContent /></Suspense>;
}
